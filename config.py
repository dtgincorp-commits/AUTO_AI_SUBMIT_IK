...
# ── LangSmith tracing ───────────────────────────────────────────────────────
# Read key via _get() so it works on both local (.env) and Streamlit Cloud
# (st.secrets). LangChain only reads os.environ, not st.secrets directly.
...
MARKETCHECK_API_KEY2 = _get("MARKETCHECK_API_KEY_2")
